<template>
    <div>
        ...
        />
        <div v-else>
            <PaginationButtons v-if="!nolimit" :paging="paging" @changePage="changePage" @changeLimit="changeLimit" />

            <MessageBox type="info" v-if="noReadPerms">
                {{
                    $gettext(
                        'Beachten Sie bitte, dass Sie hier keine Videos einbinden können, an denen Sie nur Leserechte besitzen.'
                    )
                }}
            </MessageBox>
            ...
            <table id="episodes" class="default oc--episode-table--small">
                ...
                <thead>
                    <tr class="sortable">
                        ...
                        <th></th>
                        <th v-if="showActions && !videoSortMode" class="actions" data-sort="false">
                            {{ $gettext('Aktionen') }}
                        </th>
                    </tr>
                </thead>
                    ...
                    <tr>
                        <td :colspan="numberOfColumns">
                            <span class="oc--bulk-actions">
                                <StudipButton
                                    v-if="canUpload && userHasSelectableVideos"
                                    icon="remove"
                                    @click.prevent="removeVideosFromPlaylist"
                                    :disabled="!hasCheckedVideos"
                                >
...
export default {
    ...
    components: {
        ...
        VideoRestore,
        VideoDelete,
        VideoDeletePermanent,
        VideoRemoveFromPlaylist,
        VideoEmbeddingCodeDialog,
        BulkVideoDelete,
        BulkVideoDeletePermanent,
        BulkVideoRestore,
        draggable,
    },
    ...
    computed: {
        ...
        showCheckbox() {
            return (
                (this.selectable || this.canEdit || this.canUpload || this.isContents) && this.userHasSelectableVideos
            );
        },
        ...
        userHasSelectableVideos() {
            ...
            if (this.loadedVideos.length > 0) {
                has_videos_with_write_perm =
                    this.loadedVideos.filter((v) => v.perm == 'write' || v.perm == 'owner').length > 0;
            }
    ...
    methods: {
        loadVideos() {
            ...
            if (this.isCourse && this.playlist) {
                this.$store
                    .dispatch('videos/loadPlaylistVideos', {
                        ...this.filters,
                        offset: this.offset,
                        order: this.order,
                        cid: this.cid,
                ...
                this.$store
                    .dispatch('videos/loadCourseVideos', {
                        ...this.filters,
                        offset: this.offset,
                        order: this.order,
                        cid: this.cid,
        ...
        updatePaging() {
            ...
            this.paging.lastPage =
                this.paging.items === this.limit ? 0 : Math.floor((this.paging.items - 1) / this.limit);
        },
        ...
        removePlaylist(token, cid) {
            if (confirm(this.$gettext('Sind Sie sicher, dass Sie diese Wiedergabeliste entfernen möchten?'))) {
                ...
                this.$store
                    .dispatch('playlists/removePlaylistFromCourse', {
                        token: token,
                        course: cid,
                    })
                    .then(() => {
    ...
    watch: {
        ...
        serversCheckSuccessful(success) {
            const error_msg = {
                type: 'error',
                text: this.$gettext(
                    'Es ist ein Verbindungsfehler zum Opencast Server aufgetreten und es wurden deshalb einige Funktionen deaktiviert. Bitte wenden Sie sich bei auftretenden Problemen an den Support oder versuchen Sie es zu einem späteren Zeitpunkt erneut.'
                ),
            };
