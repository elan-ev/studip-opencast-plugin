<template>
    <div>
        <ConfirmDialog v-if="showConfirmDialog"
            :title="$gettext('Bearbeiten abbrechen')"
            :message="$gettext('Sind sie sicher, dass sie das Bearbeiten abbrechen möchten?')"
            @done="decline"
...
export default {
    ...
    data () {
        return {
            ...
            upload: {
                workflow: 'upload',
                recordDate:  format(new Date(this.event['mk_date']*1000), "yyyy-MM-dd'T'HH:ii", { locale: de}), //TODO doesnt work
                subject: this.$gettext('Medienupload, Stud.IP'),
                title: this.event['title'],
                creator: this.event['author'],
                contributor: this.event['contributor'],
